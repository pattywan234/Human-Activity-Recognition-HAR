...
from GRU_model01 import create_GRU_model, var_create_GRU
...
with tf.Session() as sess:
    ...
    w_gru, b_gru = var_create_GRU(N_FEATURES, N_HIDDEN, N_CLASSES)
    pred_Y_gru = create_GRU_model(X_gru, N_FEATURES, N_HIDDEN, N_TIME_STEPS, w_gru, b_gru)
    ...
    np.save(f'wesad/all_subjects/predict_{N_EPOCHS}_lr{lr}.npy', predictions_gru)
    sess.close()

np.save(f'wesad/all_subjects/train_acc_{N_EPOCHS}_lr{lr}.npy', train_acc)
np.save(f'wesad/all_subjects/train_loss_{N_EPOCHS}_lr{lr}.npy', train_loss)
np.save(f'wesad/all_subjects/val_acc_{N_EPOCHS}_lr{lr}.npy', val_acc)
np.save(f'wesad/all_subjects/val_loss_{N_EPOCHS}_lr{lr}.npy', val_loss)
np.save(f'wesad/all_subjects/test_acc_{N_EPOCHS}_lr{lr}.npy', test_acc)
np.save(f'wesad/all_subjects/test_loss_{N_EPOCHS}_lr{lr}.npy', test_loss)

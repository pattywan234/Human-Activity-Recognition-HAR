...
from LSTM_model04 import create_LSTM_model, var_create, create_neural_net

tf.disable_v2_behavior()

#import data from PAMAP2 dataset
X_train_acc = np.load("PAMAP2/mixed/ws_pm/X_train_mix.npy")
X_test_acc = np.load("PAMAP2/mixed/ws_pm/X_test_mix.npy")
y_train_acc = np.load("PAMAP2/mixed/ws_pm/y_train_mix.npy")
y_test_acc = np.load("PAMAP2/mixed/ws_pm/y_test_mix.npy")
#import data from WESAD dataset
X_train_h = np.load('wesad/S2/X_train-acc.npy')
X_test_h = np.load('wesad/S2/X_test-acc.npy')
y_train_h = np.load('wesad/S2/y_train-acc.npy')
y_test_h = np.load('wesad/S2/y_test-acc.npy')
...
def main():
    ...
    with tf.Session() as sess:
        ...
        w_acc, b_acc = var_create(N_FEATURES_ACC, N_HIDDEN, N_CLASSES)
        ...
        loss_acc = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred_Y_acc, labels=Y_ACC)) + l2
        ...
        np.save('PAMAP2/mixed/ws_pm/after_train_acc.npy', _acc)
        sess.close()
    ...
    with tf.Session() as sess:
        ...
        w_h, b_h = var_create(N_FEATURES_H, N_HIDDEN, N_CLASSES)

        pred_Y_h = create_LSTM_model(X_H, N_FEATURES_H, N_HIDDEN, N_TIME_STEPS, w_h, b_h)
        pred_softmax_h = tf.nn.softmax(pred_Y_acc, name="y_h")

        loss_h = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred_Y_h, labels=Y_H)) + l2
        ...
        np.save('PAMAP2/mixed/ws_pm/after_train_h.npy', _h)
        sess.close()
    ...
    with tf.Session() as sess:
        ...
        w_m, b_m = var_create(N_FEATURES_H, N_HIDDEN, N_CLASSES)

        pred_Y_m = create_neural_net(X_train_mix, N_CLASSES, w_m, b_m)
        pred_softmax_m = tf.nn.softmax(pred_Y_acc, name="y_m")

        loss_m = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred_Y_m, labels=y_train_mix)) + l2
        ...
        predictions_m, final_acc_m, final_loss_m = sess.run([pred_softmax_h, accuracy_h, loss_h],

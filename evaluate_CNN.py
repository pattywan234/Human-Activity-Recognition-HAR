...
def plot_cm(cM, labels,title):
    ...
    sns.heatmap(cmNormalized, xticklabels=labels, yticklabels=labels, annot=True)
...
test_x = np.load('testData100.npy')
groundTruth = np.load('groundTruth100.npy')
...
#    predictedClass[instance] = np.argmax(predictedClass[instance,:])
#    groundTruthClass[instance] = np.argmax(groundTruthClass[instance,:])
for instance in range (groundTruth.shape[0]):
